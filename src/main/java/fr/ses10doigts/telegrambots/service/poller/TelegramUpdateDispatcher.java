...
public class TelegramUpdateDispatcher {
    ...
    public void dispatch(Update update) {
        ...
        String botId = currentBotContext.getCurrentBotId();
        TelegramUpdateContext context = TelegramUpdateContext.from(update, botId);
        if (context == null) {
            return;
        }
    ...
    private void handleUnauthorized(TelegramUpdateContext context) {
        ...
        log.warn(
                "Unauthorized Telegram user blocked: botId={}, userId={}, chatId={}",
                currentBotContext.getCurrentBotId(),
                context.getUserId(),
                context.getChatId()
        );
    }
    ...
    private void invoke(TelegramHandlerMethod handler, TelegramUpdateContext context) {
        try {
            ...
            log.error(
                    "Telegram handler threw an exception: {}#{}",
            ...
            log.error(
                    "Error while invoking Telegram handler {}#{}",
                    handler.bean().getClass().getSimpleName(),
                    handler.method().getName(),
                    e
            );
